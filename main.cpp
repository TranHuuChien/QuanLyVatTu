...
//	
		...
		void CheckMaterial()// ham nay khong co su dung chi viet roi de day thôi
		{
	...
	public:
		...
		void ReadData(){
			...
			while(1){
				...
				data>>m.SLTon;
				getline(data, temp);
//				DanhsachVatTu[cnt] = new MaterialNode(m);// cap phat vung nho them vao dau cho mang con tro
//				cnt++;	
				dem++;
...
//			editButton= Button <MaterialTab> (355,62, 160, 30, addbutton_basic_color, addbutton_oncolor, "Hieu chinh", &MaterialTab::InitEdit);
//			printButton = Button <MaterialTab> (515, 62, 170, 30, addbutton_basic_color, addbutton_oncolor, "In danh sach", &MaterialTab::InitPrint);		
			...
			//deleteButton = Button<MaterialTab> (screenWidth/2, 400, 680, 40, tab_on_color, RED, "Xoa vat tu", &MaterialTab::Delete);
	...
	public:
		...
		void WriteDataBillForAllEmployee(){
			for(int i = 0; i < nhanvien->dsNhanVien.sl; i++){
				...
				ofstream fileOut;
				fileOut.open(fileName.c_str(), ios::trunc|ios::out);
